...
**한국해양진흥공사(KOBC)**: `579cee85` — 본사 주소 오류 정정("부산 중구 중앙대로 39" → "부산광역시 해운대구 마린시티2로 38, 해운대아이파크") — 공식 사이트 오시는길 페이지 2회 조회+사람인 교차검증. branches 1→2개(본사+서울사무소, 은행로 11 일신빌딩) — 채용공고에 "일반전형 서울사무소" 모집분야가 별도 존재해 신입 배치 근무지로 확인. 런던지사·싱가포르지사는 조직상 존재는 확인되나 정확한 주소·신입 배치 여부를 공식 사이트+WebSearch 재검증까지 거쳐도 확인 못 해 `allBranches`에 "(해외)" placeholder로만 추가(KIC와 동일한 mappable 제외 패턴, §4 KIC 세션13 참고). 필기전형이 NCS 40%+전공 60% 구조임을 확인해 ncs·majorSubjects 전면 교체.
