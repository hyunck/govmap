...
### DESC_EXPANDED.md 패턴 (세션15 신규)

**파일 위치**: 프로젝트 루트 `DESC_EXPANDED.md` (커밋 `50d166d`)

description만 일괄 확장된 기관(실제 데이터 검증 없이 AI가 생성) 목록을 추적한다. 현재 **261개** (공기업 4·준정부 30·기타공공기관 227).

**Claude 자동 삭제 규칙**: 기관 데이터를 **전면 업데이트(주소·지사·NCS 등)** 완료 시, 반드시 이 파일에서 해당 기관명 행을 삭제하고 헤더의 총 개수도 갱신한다. 삭제는 data-orgs.js·orgs/ 커밋에 함께 포함.

**description patch guard 주의**: `},` 체크는 `startingSalaryHistory`/`avgSalaryHistory` 배열 아이템에 의해 오판됨 → **`/"id"\s*:\s*\d/.test(between)` 사용**
...
### 세션13 (모바일 지도 확대 토글 버그 수정 + 기관 데이터 8건: KIC·KISED·KOBC·WISET·KTO 좌표·TIPA·KEIS·HIRA)

**모바일 UI**: 세션12에서 예고된 "모바일 환경 UX 개선"을 이어감. 지도 확대(⤢) 토글 버튼 도입 후 발견된 버그 2건 수정 — 상세는 커밋 메시지 참고:
- `754ca820` 확대 모드에서 하단바(58px)가 사라졌는데도 사이드바/설명창의 `bottom`이 하드코딩값(58px)에 고정돼 그 자리가 빈 채로 남던 버그 — `--mobile-navbar-reserve` CSS 변수 도입으로 해결
- `8aa8e04a` 확대 모드에서 설명창을 X버튼으로 닫아도 잔상이 남던 버그 — `closeDetailPanel()`이 `setSheetState`가 남긴 인라인 `top`을 안 지워서 발생. 평소엔 하단바에 가려 안 보이다가 확대 모드에서 하단바가 사라지며 드러남

**한국투자공사(KIC)**: `50f1fd13` — 본사 주소 오류 정정(청계천로 100 → 퇴계로 100, 스테이트타워 남산), branches 1→7개(본사+해외사무소 6곳: 뉴욕·런던·싱가포르·도쿄·샌프란시스코·뭄바이), 신입도 K-Milestone Program으로 해외파견 가능함을 확인해 rotation 필드 신설, 시험과목이 표준 NCS가 아니라 모집분야별 전공지식평가(재무관리/통계학/경영학/전산학)임을 2025년도 채용공고 원문으로 확인해 majorSubjects·examSubjects 전면 교체(BOK와 동일하게 ncs 필드 자체를 두지 않는 패턴 적용), recruitUrl을 ALIO에서 KIC 자체 채용페이지로 교체. 이후 사용자 요청으로 해외사무소를 `branches[]`에서 빼 `allBranches[]`(본사/해외사무소 2그룹)로만 옮겨 지도 마커·클릭 대상에서 제외(`845a33b7`), 해외사무소 6곳의 실주소도 반영(`6b006049`) — **이 패턴(해외사무소 등 마커 불필요한 지사는 allBranches에만 두고 branches에서 제외)은 향후 다른 기관에도 재사용 가능**.

**창업진흥원(KISED)**: `37557668` — 본사 주소는 기존 데이터가 이미 정확함을 확인(정정 불필요). branches 1→3개(세종 본원+대전사무소·서울사무소, 둘 다 TIPS타운) — 2026년 채용공고에 "세종/대전/서울 순환근무 할 수 있음"이 명시돼 있어 신입도 실제 발령 가능한 근무지임을 확인, 주소는 KISED 자체 페이지에 없어 TIPS 프로그램 자료 기준으로 별도 WebSearch 교차검증 후 반영. rotation(권역순환) 필드 신설. 기존 majorSubjects/examSubjects(경영학·경제학)는 근거를 못 찾아 삭제하고 실제로는 NCS 5개 모듈(의사소통·수리·문제해결·자원관리·정보능력)만 있고 전공 필기 자체가 없음을 확인해 ncs 확장 + examNote로 교체. welfare는 독립 검증 실패로 추측 기재하지 않고 기존 값 유지.

**교훈**: 조사 에이전트가 "확인 안됨"을 솔직히 보고한 항목(이번엔 welfare)은 추측으로 채우지 말고 기존 값을 유지하거나 비워두는 게 맞다 — 없는 근거를 지어내는 것보다 정직한 공백이 낫다. 또한 지사 주소가 기관 자체 "오시는길" 페이지에 없고 관련 프로그램(TIPS 등) 자료에서만 나올 경우, 반드시 최소 2개 이상 독립 출처로 교차검증할 것(이번엔 WebSearch 재검증으로 대전·서울 TIPS타운 주소 확인). 이후 사용자가 알리오 공시 PDF(창업진흥원_직원+평균보수.pdf)를 제공해 연봉 히스토리 4개년(2023~2026)을 추가(`217d42d3`) — 신입사원초임 "합계"·정규직(일반정규직) "1인당 평균 보수액" 행을 천원→만원(÷10, 절사)으로 환산했고, 2025·2026년 절사값이 기존 startingSalary(4682)·avgSalary(7291)와 정확히 일치해 추출 방식을 교차검증함.

**한국해양진흥공사(KOBC)**: `579cee85` — 본사 주소 오류 정정("부산 중구 중앙대로 39" → "부산광역시 해운대구 마린시티2로 38, 해운대아이파크") — 공식 사이트 오시는길 페이지 2회 조회+사람인 교차검증. branches 1→2개(본사+서울사무소, 은행로 11 일신빌딩) — 채용공고에 "일반전형 서울사무소" 모집분야가 별도 존재해 신입 배치 근무지로 확인. 런던지사·싱가포르지사는 조직상 존재는 확인되나 정확한 주소·신입 배치 여부를 공식 사이트+WebSearch 재검증까지 거쳐도 확인 못 해 `allBranches`에 "(해외)" placeholder로만 추가(KIC와 동일한 mappable 제외 패턴, §4 KIC 세션13 참고). 이후 사용자가 2026년 공식 채용공고 PDF(붙임1.채용공고.pdf, 2026.3.5 공고)를 제공해 시험과목을 원문 기준으로 재정정(`3b243c7d`) — 처음엔 2차 출처(합격후기·블라인드 커뮤니티)로 추정 기재했던 NCS("문제해결·정보·수리능력")가 부정확했음이 드러남, 실제로는 의사소통·조직이해·수리·자원관리·문제해결 5개(직업기초능력평가 30%)+직무수행능력평가 A(40%: 민법·상법·경영학원론·회계학·재무관리)+B(30%: 금융·경영·경제·시사상식) 구조. **교훈**: 채용 사이트 접수 페이지·합격후기 등 2차 출처로 추정한 시험과목은, 사용자가 원문 공고 PDF를 제공하면 반드시 그걸로 재검증·정정할 것 — 2차 출처는 종종 다른 연도·다른 직렬 정보와 섞여 있어 부정확할 수 있음. 이후 사용자가 알리오 공시 PDF(수정공시 포함)를 제공해 연봉 히스토리 4개년(2023~2026)을 추가(`1ef69af0`) — 2025·2026년 1인당 평균 보수액이 2026.6.11 수정공시로 소폭 상향됐으나(87,406/85,885천원), 기존 avgSalary(8738)는 수정 전 원 공시값(87,381천원)과 정확히 일치해 그래프 끝점 일관성을 위해 **수정 전 값**을 기준으로 반영 — ALIO 공시 PDF에 수정 이력이 있으면 항상 어느 버전이 사용자가 이미 입력한 headline 값과 일치하는지 먼저 확인하고 그 버전으로 히스토리를 통일할 것.

**한국여성과학기술인육성재단(WISET)**: `4d33d896` — 본사 주소 오류 정정("서울 마포구 월드컵북로 400" → "서울특별시 강남구 테헤란로7길 22, 한국과학기술회관 2관 6층") — 공식 사이트 오시는길+ALIO플러스 두 독립 출처 일치로 확인. 지역본부·지사 없음(본사 단일 조직)을 확인 — "지역 여성인재 양성" 사업은 WISET 소속 지사가 아니라 위탁대학(충남대·제주대·계명대·부경대) 권역사업단으로 운영되는 것이어서 신입 배치 대상 branches에 포함하지 않음. 조사 당시엔 시험과목을 수정하지 않고 기존 값 유지했는데(WISET 채용 사이트(incruit)가 JS 렌더링 구조라 동일 URL을 WebFetch로 3회 재조회했을 때마다 NCS 모듈명이 서로 다르게 나오는 신뢰불가 신호를 확인했기 때문), 이후 사용자가 실제 채용공고 원문 3건(2025년도 4차 정규직, 2026년도 2차 행정직·공무직, 2026년도 3차 행정직)을 이미지·텍스트로 직접 제공해 `8af046b8`에서 정확히 정정 — 필기전형이 표준 NCS 모듈이 아니라 4개 모듈(의사소통·문제해결·조직이해·정보능력)만 실시하며 별도 전공 필기시험 자체가 없는 구조임을 확인, 기존 majorSubjects/examSubjects(근거 없는 추정값이었음)를 examNote로 교체.

**교훈**: 채용 사이트가 JS로 콘텐츠를 렌더링하는 구조일 경우, WebFetch가 실제 페이지 내용 대신 매번 다른(사실상 환각에 가까운) 텍스트를 반환할 수 있다 — 동일 URL을 2~3회 재조회해서 답이 매번 바뀌면 그 자체가 "신뢰 불가" 신호이므로, 그 정보로 데이터를 덮어쓰지 말고 기존 값을 유지할 것. 다만 이런 경우도 사용자가 원문 공고(이미지·PDF·텍스트)를 직접 제공하면 최우선 근거로 즉시 반영할 것 — 2차 출처 신뢰불가 문제와 원문 확보는 별개이며, 원문이 생기면 곧바로 정정해야 함.

**한국관광공사(KTO)**: 사용자가 해외지사 좌표를 하나씩 직접 제공(`94ef702e` 후쿠오카, `be25010d` 오사카, `22662ef6` 베이징·선양, `1eec0cf6` 블라디보스토크) — 30개 해외지사 중 좌표가 확보된 곳만 개별적으로 `branches[]`에 lat/lng 직접 지정으로 추가해 지도 마커·클릭(📍) 대상으로 승격시키고, 나머지는 `allBranches[]`에만 남겨 목록 전용(비마커) 유지. **패턴**: branches[]와 allBranches[] 양쪽의 name+address를 정확히 일치시켜야 isMapped 매칭이 유지됨 — 주소 텍스트를 나중에 갱신할 땐(`b5603006`, 30개 해외지사 실주소 전면 반영) 이미 좌표가 붙은 지사(후쿠오카)의 branches[]/allBranches[] 주소도 함께 동기화해야 마커 연결이 깨지지 않음.

**중소기업기술정보진흥원(TIPA)**: `505ab01f` — 본사 주소 오류 정정("서울 강남구 논현로 432"(구 소재지) → "세종특별자치시 집현중앙로 79") — 2021년 5월 세종 이전 사실이 기존 데이터에 반영 안 돼 있었음, region도 수도권→충청권으로 동시 정정(주소만 바꾸고 region을 놓치기 쉬우니 항상 함께 확인할 것). branches는 본원 단일 유지, allBranches에 부설기관(스마트제조혁신추진단, 본원과 동일건물)과 수도권 부속실 3곳을 추가 — 채용공고에 "본원 및 부설기관 순환근무"가 명시돼 있어 rotation 필드로 반영했지만, 수도권 부속실 3곳은 신입 배치 근거를 못 찾아 목록 전용(비마커)으로만 추가. NCS가 직렬(일반직 4급 8영역 vs 연구지원직 7영역, 자원관리능력 유무 차이)별로 다름을 확인해 ncsOld로 세분화. 이후 사용자가 "지도에 추가 안된 얘들도 표시되게 해줘"라고 요청해(`32e0c402`) 신입 배치 불확실성과 무관하게 4곳 전부 branches[]로 승격, 알리오 공시 PDF로 연봉 히스토리 4개년도 추가(`e2e95837`).

**한국고용정보원(KEIS)**: `c1f4879a` — 본원 주소는 기존 데이터가 정확함을 재확인. branches의 기존 "서울청사"(은행로 30) 항목이 근거 없는 오기재였음을 확인해 제거 — 공식 오시는길 페이지·ALIOPLUS 사업장 목록·복수 채용공고 모두 서울 사업장을 언급하지 않아 다수 독립 출처로 부재 확인(WebSearch 한 건이 "문래동 서울청사"를 언급했으나 재검색 2회로 재현 안 돼 신뢰불가로 판단, 다른 기관과 혼동된 것으로 추정). 대신 실제 확인된 국가고용정보화개발센터·지역일자리지원센터(오송) 2곳을 공식 하위 페이지에서 직접 주소 인용해 추가. 시험과목은 첨부파일이 zip/이미지라 원문 확인 실패로 기존 값 유지.

**교훈**: "○○청사가 존재한다"는 WebSearch 요약 한 줄만으로 판단하지 말 것 — 검색 요약은 여러 기관이 섞인 결과를 뭉뚱그려 잘못 합성할 수 있다(이번엔 한국고용노동교육원·서울고용센터 등 유사 명칭 기관과 혼동 가능성). 존재를 주장하는 단서가 나오면 같은 질의를 다른 표현으로 2회 이상 재검색해 재현되는지 확인하고, 안 되면 공식 사이트의 명시적 목록(오시는길 탭, 조직도, ALIOPLUS 사업장 정보)을 최종 근거로 삼을 것. 또한 이번 세션에서 general-purpose 리서치 에이전트가 "조사해줘"라는 지시만 받고도 자체적으로 data-orgs.js를 수정하고 페이지까지 재생성한 사례가 있었음 — 결과물은 검증 결과 정확했지만, 순수 조사만 원할 땐 프롬프트에 "파일 수정 금지, 조사 결과만 보고"를 명시하는 게 안전함.

**건강보험심사평가원(HIRA)**: `6172515a` — branches의 기존 "서울지원"(효령로 267)·"부산지원"(센텀동로 99) 주소가 완전히 틀려있었음(서울은 2019년 송파구로 이미 이전). 본원+지역본부 12개(서울·부산·대구경북·전남광주·대전충청·경기남부·울산경남·경기북부·전북·인천·강원·제주) = 총 13개소로 전면 교체, allBranches도 5개 권역 그룹으로 정리.
...
### 세션15 (description 일괄 확장 261개 + DESC_EXPANDED.md)

| 작업 | 커밋 | 주요 변경 |
|------|------|-----------|
| description 일괄 확장 (공기업·준정부 34개) | — | `_patch_desc.js` — `},` guard 오작동 버그(startingSalaryHistory 배열의 `},`를 entry 경계로 오판) → `/"id"\s*:\s*\d/` guard로 수정. 공기업 4개·준정부 30개 OK |
| description 일괄 확장 (기타공공기관 227개) | — | `_patch_desc2a/2b/2c.js` 3분할 실행. 227개 전부 OK. 전체 기관 0개 남음(100자 미만 기관 없음) |
| DESC_EXPANDED.md 생성·커밋 | `50d166d` | 261개 기관 목록 추적 파일. 기관 전면 업데이트 시 해당 기관명 자동 삭제 (§3 DESC_EXPANDED 패턴 참고) |

**교훈 (description patch guard 수정)**: `},` 체크는 `startingSalaryHistory`/`avgSalaryHistory` 배열 아이템(`{"year":2023,"val":X},`)에 의해 오판된다. **반드시 `/"id"\s*:\s*\d/.test(between)` 사용** — `"id":` 필드는 각 entry의 `"name":` 앞에만 나오므로 entry 경계 감지에 정확함.
...
### 현재 작업 위치
- **마지막 완료**: 세션15 — description 일괄 확장(공기업 4·준정부 30·기타공공기관 227 = 261개) + DESC_EXPANDED.md 생성·커밋 + 애드센스 재심사 요청(검토 진행 중). §4 세션15 참고.
- **세션15에서 완료한 전체 목록** (시간순): description patch guard 버그 수정(`},` → `"id":`) → 공기업·준정부 34개 확장 → 기타공공기관 227개 확장(2a/2b/2c) → DESC_EXPANDED.md 생성(`50d166d`) → 애드센스 재심사 요청(2026-07-29 오후 5:21 기준 "준비 중")
...
| ~~한국고용정보원 (KEIS, 음성)~~ | — | — | ✅ 세션13에서 완료 |
| ~~건강보험심사평가원 (HIRA, 원주)~~ | — | — | ✅ 세션13에서 완료 |
| 이후 계속 | — | — | 사용자 요청에 따라 결정 |
...
### 현재 상태 (2026-07-29 기준)
- **애드센스 사이트 상태**: "준비 중" (재심사 요청 완료, 검토 진행 중)
- **Ads.txt**: 대시보드에 "찾을 수 없음"으로 표시되나 `govmap.kr/ads.txt` 직접 접속 시 정상 서빙 확인 — 구글 크롤러가 아직 재확인 안 한 것이므로 무시해도 됨
- **지급 정보**: 미입력 상태이나 검토와 무관 — 승인 후에도 입력 가능
- **대기 중**: 검토 결과 기다리는 중. 별도 조치 불필요
...
5. **description 일괄 확장 261개 (세션15)** — 공기업 4·준정부 30·기타공공기관 227개의 description을 2문장으로 확장. `DESC_EXPANDED.md`(`50d166d`)로 추적 중
...
- **애드센스 검토 크롤러(AdsBot-Google)는 라이브 사이트에 직접 접속** — 서치 인덱스 캐시를 보는 게 아님. 따라서 변경사항을 배포하면 바로 재신청해도 됨. Search Console sitemap 재제출은 애드센스 검토와 무관
...
- **지급 정보 미입력은 검토에 영향 없음** — 사이트 승인과 완전히 별개. 승인 후 입력해도 됨
