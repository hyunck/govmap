...
**교훈**: 조사 에이전트가 "확인 안됨"을 솔직히 보고한 항목(이번엔 welfare)은 추측으로 채우지 말고 기존 값을 유지하거나 비워두는 게 맞다 — 없는 근거를 지어내는 것보다 정직한 공백이 낫다. 또한 지사 주소가 기관 자체 "오시는길" 페이지에 없고 관련 프로그램(TIPS 등) 자료에서만 나올 경우, 반드시 최소 2개 이상 독립 출처로 교차검증할 것(이번엔 WebSearch 재검증으로 대전·서울 TIPS타운 주소 확인).
...
### 현재 작업 위치
- **마지막 완료**: 창업진흥원(KISED) 전면 업데이트 (세션13) — §4 세션13 참고. 그 직전엔 한국투자공사(KIC), 그보다 앞서 모바일 지도 확대 토글 버그 2건 수정
