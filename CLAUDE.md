...
**한국여성과학기술인육성재단(WISET)**: `4d33d896` — 본사 주소 오류 정정("서울 마포구 월드컵북로 400" → "서울특별시 강남구 테헤란로7길 22, 한국과학기술회관 2관 6층") — 공식 사이트 오시는길+ALIO플러스 두 독립 출처 일치로 확인. 지역본부·지사 없음(본사 단일 조직)을 확인 — "지역 여성인재 양성" 사업은 WISET 소속 지사가 아니라 위탁대학(충남대·제주대·계명대·부경대) 권역사업단으로 운영되는 것이어서 신입 배치 대상 branches에 포함하지 않음. **시험과목은 수정하지 않고 기존 값 유지** — WISET 채용 사이트(incruit)가 JS 렌더링 구조라 동일 URL을 WebFetch로 3회 재조회했을 때마다 NCS 모듈명이 서로 다르게 나오는(의사소통/수리/문제해결/자원관리 → NCS기반 직무능력검사만 → 언어/수리/문제해결/자료해석 등) 명백한 신뢰불가 신호를 확인해 반영 보류.

**교훈**: 채용 사이트가 JS로 콘텐츠를 렌더링하는 구조일 경우, WebFetch가 실제 페이지 내용 대신 매번 다른(사실상 환각에 가까운) 텍스트를 반환할 수 있다 — 동일 URL을 2~3회 재조회해서 답이 매번 바뀌면 그 자체가 "신뢰 불가" 신호이므로, 그 정보로 데이터를 덮어쓰지 말고 기존 값을 유지할 것.
